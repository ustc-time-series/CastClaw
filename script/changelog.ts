...
import { $ } from "bun"
import { createCastClaw } from "@castclaw/sdk/v2"
import { parseArgs } from "util"
import { Script } from "@castclaw/script"
...
export async function getLatestRelease(skip?: string) {
  const data = await fetch("https://api.github.com/repos/SkyeGT/CastClaw/releases?per_page=100").then((res) => {
...
export async function getCommits(from: string, to: string): Promise<Commit[]> {
  ...
  const compare =
    await $`gh api "/repos/SkyeGT/CastClaw/compare/${fromRef}...${toRef}" --jq '.commits[] | {sha: .sha, login: .author.login, message: .commit.message}'`.text()
  ...
  const log =
    await $`git log ${fromRef}..${toRef} --oneline --format="%H" -- packages/castclaw packages/sdk packages/plugin packages/app packages/web`.text()
  ...
  for (const hash of hashes) {
    ...
    for (const file of files.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/castclaw/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/castclaw/")) areas.add("core")
      else if (file.startsWith("packages/app/")) areas.add("web")
      else if (file.startsWith("packages/web/")) areas.add("web")
...
function getSection(areas: Set<string>): string {
  ...
  const priority = ["core", "tui", "web", "sdk", "plugin"]
...
async function summarizeCommit(castclaw: Awaited<ReturnType<typeof createCastClaw>>, message: string): Promise<string> {
  console.log("summarizing commit:", message)
  const session = await castclaw.client.session.create()
  const result = await castclaw.client.session
    .prompt(
      {
        sessionID: session.data!.id,
        model: { providerID: "castclaw", modelID: "claude-sonnet-4-5" },
        tools: {
          "*": false,
        },
        parts: [
...
export async function generateChangelog(commits: Commit[], castclaw: Awaited<ReturnType<typeof createCastClaw>>) {
  // Summarize commits in parallel with max 10 concurrent requests
  const BATCH_SIZE = 10
  const summaries: string[] = []
  for (let i = 0; i < commits.length; i += BATCH_SIZE) {
    const batch = commits.slice(i, i + BATCH_SIZE)
    const results = await Promise.all(batch.map((c) => summarizeCommit(castclaw, c.message)))
  ...
  const sectionOrder = ["Core", "TUI", "Web", "SDK"]
  const lines: string[] = []
...
export async function getContributors(from: string, to: string) {
  ...
  const toRef = to === "HEAD" ? to : to.startsWith("v") ? to : `v${to}`
  const compare =
    await $`gh api "/repos/SkyeGT/CastClaw/compare/${fromRef}...${toRef}" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
export async function buildNotes(from: string, to: string) {
  ...
  const castclaw = await createCastClaw({ port: 0 })
  const notes: string[] = []

  try {
    const lines = await generateChangelog(commits, castclaw)
    notes.push(...lines)
