...
const model = "castclaw/gpt-5.3-codex"
...
async function typecheck() {
  ...
  try {
    await $`bun typecheck`.cwd("packages/castclaw")
    return true
  } catch (err) {
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/castclaw")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with castclaw...`)
  ...
  const prompt = [
    ...
    "After resolving the conflicts, run `bun typecheck` in `packages/castclaw`.",
  ...
  try {
    await $`castclaw run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  castclaw failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with castclaw")
  return true
}

async function smoke(prs: PR[], applied: number[]) {
  console.log("\nRunning final smoke check with castclaw...")

  const done = lines(prs.filter((x) => applied.includes(x.number)))
  const prompt = [
    "The beta merge batch is complete.",
    `Merged PRs on HEAD:\n${done}`,
    "Run `bun typecheck` in `packages/castclaw`.",
    "Run `./script/build.ts --single` in `packages/castclaw`.",
