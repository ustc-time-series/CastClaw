...
const managedConfigDir = process.env.CASTCLAW_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "castclaw.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "castclaw.json") {
...
async function check(map: (dir: string) => string) {
  ...
  try {
    await writeConfig(globalTmp.path, {
      $schema: "https://raw.githubusercontent.com/SkyeGT/CastClaw/dev/packages/web/public/config.json",
      snapshot: false,
    })
...
test("ignores legacy tui keys in castclaw config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://raw.githubusercontent.com/SkyeGT/CastClaw/dev/packages/web/public/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "castclaw.json"))
...
test("resolves env templates in account config with account token", async () => {
  ...
  const originalControlToken = process.env["CASTCLAW_CONSOLE_TOKEN"]
  ...
  Account.config = mock(async () => ({
    provider: {
      castclaw: {
        options: {
          apiKey: "{env:CASTCLAW_CONSOLE_TOKEN}",
        },
      },
    },
  }))
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        const config = await Config.get()
        expect(config.provider?.["castclaw"]?.options?.apiKey).toBe("st_test_token")
      },
    })
  } finally {
    ...
    if (originalControlToken !== undefined) {
      process.env["CASTCLAW_CONSOLE_TOKEN"] = originalControlToken
    } else {
      delete process.env["CASTCLAW_CONSOLE_TOKEN"]
    }
  }
})
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "castclaw.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .castclaw directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const castclawDir = path.join(dir, ".castclaw")
      await fs.mkdir(castclawDir, { recursive: true })
      const agentDir = path.join(castclawDir, "agent")
...
test("loads agents from .castclaw/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const castclawDir = path.join(dir, ".castclaw")
      await fs.mkdir(castclawDir, { recursive: true })

      const agentsDir = path.join(castclawDir, "agents")
...
test("loads commands from .castclaw/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const castclawDir = path.join(dir, ".castclaw")
      await fs.mkdir(castclawDir, { recursive: true })

      const commandDir = path.join(castclawDir, "command")
...
test("loads commands from .castclaw/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const castclawDir = path.join(dir, ".castclaw")
      await fs.mkdir(castclawDir, { recursive: true })

      const commandsDir = path.join(castclawDir, "commands")
...
test("does not try to install dependencies in read-only CASTCLAW_CONFIG_DIR", async () => {
  if (process.platform === "win32") return
  ...
  const prev = process.env.CASTCLAW_CONFIG_DIR
  process.env.CASTCLAW_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.CASTCLAW_CONFIG_DIR
    else process.env.CASTCLAW_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable CASTCLAW_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "castclaw.json"),
        JSON.stringify({ $schema: "https://raw.githubusercontent.com/SkyeGT/CastClaw/dev/packages/web/public/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "castclaw.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .castclaw config
      const projectDir = path.join(dir, "project")
      const castclawDir = path.join(projectDir, ".castclaw")
      await fs.mkdir(castclawDir, { recursive: true })
      ...
      // Local .castclaw config with different plugins
      await Filesystem.write(
        path.join(castclawDir, "castclaw.json"),
        JSON.stringify({
          $schema: "https://raw.githubusercontent.com/SkyeGT/CastClaw/dev/packages/web/public/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .castclaw config with some overlapping plugins
      await Filesystem.write(
        path.join(castclawDir, "castclaw.json"),
        JSON.stringify({
          $schema: "https://raw.githubusercontent.com/SkyeGT/CastClaw/dev/packages/web/public/config.json",
...
// Note: preload.ts sets CASTCLAW_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .castclaw config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .castclaw directory config enables it
      const castclawDir = path.join(dir, ".castclaw")
      await fs.mkdir(castclawDir, { recursive: true })
      await Filesystem.write(
        path.join(castclawDir, "castclaw.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/castclaw")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/castclaw")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-castclaw@2.4.3")).toBe("oh-my-castclaw")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@castclaw/plugin@2.0.0")).toBe("@castclaw/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-castclaw@2.4.3", "file:///project/.castclaw/plugin/oh-my-castclaw.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.castclaw/plugin/oh-my-castclaw.js")
  })
  ...
  test("local plugin directory overrides global castclaw.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const castclawDir = path.join(projectDir, ".castclaw")
        const pluginDir = path.join(castclawDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("CASTCLAW_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["CASTCLAW_DISABLE_PROJECT_CONFIG"]
    process.env["CASTCLAW_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["CASTCLAW_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["CASTCLAW_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .castclaw/ directories when flag is set", async () => {
    const originalEnv = process.env["CASTCLAW_DISABLE_PROJECT_CONFIG"]
    process.env["CASTCLAW_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .castclaw directory with a command
          const castclawDir = path.join(dir, ".castclaw", "command")
          await fs.mkdir(castclawDir, { recursive: true })
          await Filesystem.write(path.join(castclawDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .castclaw should NOT be in directories list
          const hasProjectCastClaw = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectCastClaw).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["CASTCLAW_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["CASTCLAW_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["CASTCLAW_CONFIG_DIR"]
      process.env["CASTCLAW_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["CASTCLAW_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["CASTCLAW_CONFIG_DIR"]
      } else {
        process.env["CASTCLAW_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("CASTCLAW_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["CASTCLAW_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["CASTCLAW_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["CASTCLAW_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["CASTCLAW_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from CASTCLAW_CONFIG_DIR, not project
...
describe("CASTCLAW_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in CASTCLAW_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["CASTCLAW_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["CASTCLAW_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://raw.githubusercontent.com/SkyeGT/CastClaw/dev/packages/web/public/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["CASTCLAW_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["CASTCLAW_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in CASTCLAW_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["CASTCLAW_CONFIG_CONTENT"]
