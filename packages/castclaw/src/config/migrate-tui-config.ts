...
const TUI_SCHEMA_URL = "https://raw.githubusercontent.com/SkyeGT/CastClaw/dev/packages/web/public/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from castclaw.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const castclaw = await castclawFiles(input)
  for (const file of castclaw) {
...
async function castclawFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.CASTCLAW_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("castclaw", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "castclaw")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "castclaw"))
  }
  if (Flag.CASTCLAW_CONFIG) files.push(Flag.CASTCLAW_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "castclaw"))
