...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.CASTCLAW_TUI_CONFIG
  }

  const state = Instance.state(async () => {
    let projectFiles = Flag.CASTCLAW_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.CASTCLAW_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!dir.endsWith(".castclaw") && dir !== Flag.CASTCLAW_CONFIG_DIR) continue
  ...
  async function load(text: string, configFilepath: string): Promise<Info> {
    ...
    // (mirroring the old castclaw.json shape) still get their settings applied.
    const normalized = (() => {
