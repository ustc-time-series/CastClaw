...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run castclaw",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for castclaw session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found castclaw session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["castclaw", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting castclaw...")
        UI.println()

        const castclawArgs = sessionId ? ["-s", sessionId] : []
        const castclawProcess = Process.spawn(["castclaw", ...castclawArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await castclawProcess.exited
        if (code !== 0) throw new Error(`castclaw exited with code ${code}`)
      },
    })
  },
})
