...
const AGENT_USERNAME = "castclaw-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/castclaw.yml"
      ...
      async fn() {
        {
          ...
          function printNextSteps() {
            ...
            prompts.outro(
              [
                ...
                step2,
                "",
                "    3. Go to a GitHub issue and comment `/castclaw summarize` to see the agent in action",
                "",
                "   Learn more about the GitHub agent - https://github.com/SkyeGT/CastClaw/github/#usage-examples",
              ].join("\n"),
            )
          }
          ...
          async function installGitHubApp() {
            ...
            // Open browser
            const url = "https://github.com/apps/castclaw-agent"
            const command =
            ...
            async function getInstallation() {
              throw new Error("GitHub App installation check requires a configured backend (OIDC_BASE_URL)")
            }
          }
...
jobs:
  castclaw:
    if: |
      contains(github.event.comment.body, ' /castclaw') ||
      startsWith(github.event.comment.body, '/castclaw')
    ...
    steps:
      ...
      - name: Run castclaw
        uses: SkyeGT/CastClaw/github@latest${envStr}
        with:
...
export const GithubRunCommand = cmd({
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      const shareBaseUrl = "https://github.com/SkyeGT/CastClaw"
      ...
      try {
        ...
        console.log("castclaw session", session.id)
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) throw new Error("OIDC_BASE_URL environment variable is required for GitHub Actions integration")
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/castclaw")
          .split(",")
      ...
      async function chat(message: string, files: PromptFiles = []) {
        console.log("Sending message to castclaw...")
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("castclaw-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `castclaw/${type}-${hex}-${timestamp}`
        }
        return `castclaw/${type}${issueId}-${timestamp}`
      }
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/castclaw-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
        const shareUrl = shareId ? `[castclaw session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the castclaw infrastructure after your response",
