...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall castclaw and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall CastClaw")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g castclaw-ai",
      pnpm: "pnpm uninstall -g castclaw-ai",
      bun: "bun remove -g castclaw-ai",
      yarn: "yarn global remove castclaw-ai",
      brew: "brew uninstall castclaw",
      choco: "choco uninstall castclaw",
      scoop: "scoop uninstall castclaw",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "castclaw", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using CastClaw!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# castclaw") || content.includes(".castclaw/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".castclaw/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".castclaw/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".castclaw"))
    ) {
      continue
    }
