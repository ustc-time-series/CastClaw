...
import type { AssistantMessage, Event, CastClawClient, SessionMessageResponse, ToolPart } from "@castclaw/sdk/v2"
...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: CastClawClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `castclaw auth login` in the terminal",
        name: "Login with castclaw",
        id: "castclaw-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // CastClaw stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
    ...
    const castclawProvider = providers.find((p) => p.id === "castclaw")
    if (castclawProvider) {
      if (castclawProvider.models["big-pickle"]) {
        return { providerID: ProviderID.castclaw, modelID: ModelID.make("big-pickle") }
      }
      const [best] = Provider.sort(Object.values(castclawProvider.models))
      if (best) {
        return {
