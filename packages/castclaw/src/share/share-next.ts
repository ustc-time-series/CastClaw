...
import type * as SDK from "@castclaw/sdk/v2"
...
export namespace ShareNext {
  ...
  const orgApi = apiEndpoints("shares")
  ...
  export async function request(): Promise<{
    ...
    if (!active?.active_org_id) {
      ...
      if (!baseUrl) throw new Error("Session sharing requires enterprise config (enterprise.url)")
    ...
    return { headers, api: orgApi, baseUrl: active.url }
  }

  const disabled = process.env["CASTCLAW_DISABLE_SHARE"] === "true" || process.env["CASTCLAW_DISABLE_SHARE"] === "1"
