import { Slug } from "@castclaw/util/slug"
...
export namespace Session {
  ...
  export async function createNext(input: {
    ...
    if (!result.parentID && (Flag.CASTCLAW_AUTO_SHARE || cfg.share === "auto")) {
      share(result.id).catch(() => {
        // Silently ignore sharing errors during session creation
      })
    }

    if (!Flag.CASTCLAW_EXPERIMENTAL_WORKSPACES) {
  ...
  export function plan(input: { slug: string; time: { created: number } }) {
    const base = Instance.project.vcs
      ? path.join(Instance.worktree, ".castclaw", "plans")
  ...
  export const getUsage = fn(
    ...
    (input) => {
      ...
      // It looks like CastClaw's cost calculation assumes all providers return inputTokens the same way Anthropic does (I'm guessing getUsage logic was originally implemented with anthropic), so it's causing incorrect cost calculation for OpenRouter and others.
