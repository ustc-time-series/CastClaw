...
import castclawThemeJson from "./themes/castclaw.json"
...
const STORAGE_KEYS = {
  THEME_ID: "castclaw-theme-id",
  COLOR_SCHEME: "castclaw-color-scheme",
  THEME_CSS_LIGHT: "castclaw-theme-css-light",
  THEME_CSS_DARK: "castclaw-theme-css-dark",
} as const

const THEME_STYLE_ID = "castclaw-theme"
...
const castclawTheme = castclawThemeJson as DesktopTheme

function normalize(id: string | null | undefined) {
  return id === "oc-1" || id === "oc-2" ? "castclaw" : id
}
...
function applyThemeCss(theme: DesktopTheme, themeId: string, mode: "light" | "dark") {
  ...
  document.getElementById("castclaw-theme-preload")?.remove()
...
export const { use: useTheme, provider: ThemeProvider } = createSimpleContext({
  ...
  init: (props: { defaultTheme?: string; onThemeApplied?: (theme: DesktopTheme, mode: "light" | "dark") => void }) => {
    const themeId = normalize(read(STORAGE_KEYS.THEME_ID) ?? props.defaultTheme) ?? "castclaw"
    const colorScheme = (read(STORAGE_KEYS.COLOR_SCHEME) as ColorScheme | null) ?? "system"
    const mode = colorScheme === "system" ? getSystemMode() : colorScheme
    const [store, setStore] = createStore({
      themes: {
        castclaw: castclawTheme,
    ...
    const onStorage = (e: StorageEvent) => {
      if (e.key === STORAGE_KEYS.THEME_ID && e.newValue) {
        ...
        if (!next) return
        if (next !== "castclaw" && !knownThemes().has(next) && !store.themes[next]) return
    ...
    onMount(() => {
      ...
      const savedTheme = normalize(rawTheme ?? props.defaultTheme) ?? "castclaw"
    ...
    const setTheme = (id: string) => {
      ...
      if (next !== "castclaw" && !knownThemes().has(next) && !store.themes[next]) {
        console.warn(`Theme "${id}" not found`)
        return
      }
      setStore("themeId", next)
