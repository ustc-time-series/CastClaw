...
test("changing language updates settings labels", async ({ page, gotoSession }) => {
  await page.addInitScript(() => {
    localStorage.setItem("castclaw.global.dat:language", JSON.stringify({ locale: "en" }))
  })
...
test("changing theme persists in localStorage", async ({ page, gotoSession }) => {
  ...
  const storedThemeId = await page.evaluate(() => {
    return localStorage.getItem("castclaw-theme-id")
  })
...
test("legacy theme migrates to castclaw", async ({ page, gotoSession }) => {
  await page.addInitScript(() => {
  ...
  await expect(page.locator("html")).toHaveAttribute("data-theme", "castclaw")
  ...
  await expect
    .poll(async () => {
      return await page.evaluate(() => {
        return localStorage.getItem("castclaw-theme-css-light")
      })
    })
    .toBeNull()
  ...
  await expect
    .poll(async () => {
      return await page.evaluate(() => {
        return localStorage.getItem("castclaw-theme-css-dark")
      })
    })
    .toBeNull()
})
