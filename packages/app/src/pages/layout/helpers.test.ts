...
import { type Session } from "@castclaw/sdk/v2/client"
import {
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("castclaw://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("castclaw://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("castclaw://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("castclaw://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("castclaw://open-project")).toBeUndefined()
    expect(parseDeepLink("castclaw://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "castclaw://open-project?directory=/a",
      "castclaw://other?directory=/b",
      "castclaw://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("castclaw://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("castclaw://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("castclaw://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("castclaw://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "castclaw://new-session?directory=/a",
      "castclaw://open-project?directory=/b",
      "castclaw://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __CASTCLAW__: {
        deepLinks: ["castclaw://open-project?directory=/a"],
      },
    } as unknown as Window & { __CASTCLAW__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["castclaw://open-project?directory=/a"])
